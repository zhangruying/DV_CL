...
class TwoLayerNet(object):
    ...
    def __init__(
        ...
        # 第一层affine：输入（N,D）
        # 权重W1：(D,H) ——> X@W = (N,H)
        # 偏置b1：(H,)或(1,H) 广播加到每一行  
        # 第二层affine：
        # 输入(N,H)     (来自ReLU输出)    
        # 权重W2:(H,C) ——>  (N,H)@(H,C) ——> (N,C)    
        # 偏置b2：(C,)       
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        # 反向传播需要计算损失函数对每个参数的梯度,loss = data_loss + reg_loss，求W导要对两项损失都求导！！！
        ...
        da1 = dh1.copy()
        da1[a1 <= 0] = 0
        # da1 = dh1 *(a1 > 0)  # eg:  (a1>0) = [False,True,False,True] ——> 转化为int: [0,1,0,1]
